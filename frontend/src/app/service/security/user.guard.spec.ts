...
describe('UserGuard', () => {
  ...
  it('should allow access for authenticated regular users', (done) => {
    mockAuthService.checkSession.and.returnValue(of(true));
    mockAuthService.isAdmin.and.returnValue(false);

    TestBed.runInInjectionContext(() => {
      const result = UserGuard(null as any, null as any) as Observable<any>;
  ...
  it('should redirect to admin panel for authenticated admin users', (done) => {
    const adminUrl = '/admin';
    mockAuthService.checkSession.and.returnValue(of(true));
    mockAuthService.isAdmin.and.returnValue(true);
    mockRouter.parseUrl.and.returnValue(adminUrl as any);

    TestBed.runInInjectionContext(() => {
      const result = UserGuard(null as any, null as any) as Observable<any>;
      
      result.subscribe((canActivate: any) => {
        expect(canActivate).toBe(adminUrl as any);
        expect(mockAuthService.checkSession).toHaveBeenCalled();
        expect(mockAuthService.isAdmin).toHaveBeenCalled();
